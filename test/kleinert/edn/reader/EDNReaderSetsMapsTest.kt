...
class EDNReaderSetsMapsTest {
    ...
    fun parseWithConverter() {
        run {
            val options = EDN.defaultOptions.copy(listToEdnMapConverter = { IdentityHashMap(it.toMap()) })
        ...
        run {
            val options = EDN.defaultOptions.copy(listToEdnSetConverter = { TreeSet<Long>() })
            val parsed = EDN.read("#{1 2}", options)
